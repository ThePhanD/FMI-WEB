...
class Db {
	...
	private function init($dbhost, $dbName, $userName, $userPassword) {
		try {
			...
			$this->insertRoomStatement->execute(["room_name" => 'room101', "rowNumber" => 5, "colNumber" => 4, "creator" => 'admin_user',
                                                 "music" => 'WebIsTheBest', "places" => '11110100111001101101' , "isActive" => 0 ]);
	...
	private function prepareStatements() {
		...
		$sql = "DELETE FROM rooms WHERE room_name=:room_name";
		$this->deleteRoomsStatement = $this->connection->prepare($sql);
	...
	public function deleteRoomQuery($data) {
		try {
			$this->deleteRoomsStatement->execute($data);
